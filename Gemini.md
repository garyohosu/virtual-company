...
# Step 2: ルートの設定ファイルを読む
echo "📖 設定ファイルを読み込み中..."
source ./Agents.md      # すべてのエージェント共通ルール
source ./Claude.md      # Claude の設定
source ./Gemini.md      # Gemini の設定
...
Agents.md             ← ルール定義（ルートにある）
Claude.md             ← ルール定義（ルートにある）
Gemini.md             ← ルール定義（ルートにある）
_*.md                 ← 下書きファイル
_*.sh                 ← 下書きスクリプト
```
...
□ 新しいファイルを検出
□ 設定ファイル（CLAUDE_MEMORY.md など）をスキップ
□ .sh ファイルを bash で実行
□ .md ファイルを読んで表示
